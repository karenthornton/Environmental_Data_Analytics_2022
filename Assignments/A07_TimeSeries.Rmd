...
```{r}
#7
ggplot(GaringerOzone2, aes(x= Date, y= Daily.Max.8.hour.Ozone.Concentration))+
  geom_line()+
...
> Answer: If you take out the seasonality the time series is still statistically significant. The p-value is 0.04965 which is less than 0.05, meaning the ozone has changed significantly since 2010, even when you look at the data without seasonality. ???
...
```{r}
#15
Garinger_Components <- as.data.frame(GaringerOzone.monthly.ts$time.series[,1:3])
