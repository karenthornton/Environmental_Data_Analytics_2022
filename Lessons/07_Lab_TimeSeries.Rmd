...
Temp_BR_processed <-
  ...
  separate(Statistics,c("Null","Month","Null2")," ") %>% 
  #quotes after the function mean that you want to separate by a space
  mutate( Date = my(paste0(Month,"-",Year))) %>% 
  #paste0 pastes the information together
...
summary(Rainfall_BR_processed)

summary(Temp_BR_processed)

Rainfallplot <-ggplot(Rainfall_BR_processed,aes(x=Date,y=Rainfall))+
  geom_line()+
  #use geom_line for time series
  geom_smooth(method = "lm")
print(Rainfallplot)

Tempplot <- ggplot(Temp_BR_processed, aes(x=Date, y=Temperature_C))+
  geom_line()+
  geom_smooth(method = "lm")
print(Tempplot)
...
Rainfall_ts <- ts(Rainfall_BR_processed$Rainfall, start= c(1,1901), frequency = 12)

Temp_ts <- ts(Temp_BR_processed$Temperature_C, start = c(1,1901), frequency = 12)
...
Rainfall_Components <- 
  as.data.frame(Rainfall_decomposed$time.series[,1:3])
Temp_Components <-
  as.data.frame(Temp_decomposed$time.series[,1:3])

Rainfall_Components2 <- mutate(Rainfall_Components)
