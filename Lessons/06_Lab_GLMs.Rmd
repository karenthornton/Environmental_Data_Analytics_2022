...
EPA1<-ggplot(EPAair, aes(x=Ozone, fill=Year))+
  #can put the as.factor after fill too
  geom_density(alpha = 0.5, size =1)+
  scale_fill_manual(values = c("darkgreen", "darkblue"))+
  labs(x= "Ozone AQI value", y= "Density", fill = "")
# the empty "" making sure that the legend has no name
print(EPA1)

EPA2 <-ggplot(EPAair, aes(x=Year, y=Ozone))+
  geom_boxplot()+
  stat_summary(geom = "text", fun.y = max, vjust= -1, size=4,
               #1 instead of -1 would put the label below the max
               #vertical ajustment, fun.y =max puts the a and b at the top (could also put min, mean, median, etc.)
               #everything in the stats_sum is for the label
               label=c("b","a"))+
  labs(x= "", y="Ozone AQI value")+
  ylim(0,150)
print(EPA2)

EPA3 <-ggplot(EPAair, aes(x=Year, y=Ozone))+
...
#Reg1<-lm(EPAair$PM2.5 ~ EPAair$Ozone)
#summary(Reg1)
...
#ggplot(Reg1, aes(x=Ozone, y = PM2.5))+
 # geom_point()+
...
EPAAIC <- lm(data= EPAair.subset, PM2.5 ~ Ozone + Year + Month + meanLat + meanLng)
...
EPAmodel <- lm(data = EPAair.subset, PM2.5 ~ Ozone + Month + meanLat + meanLng)
summary(EPAmodel)
