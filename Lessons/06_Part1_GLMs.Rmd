...
```{r}
irradiance.regression <- lm(PeterPaul.chem.nutrients$irradianceWater ~ PeterPaul.chem.nutrients$depth) #y then x variable??
...
#our plot has more on the right side
...
step(TPAIC)
# if the variable has a lower AIC then the "none" then they should not be included. so DONT remove any variable that make the AIC larger than the orginal AIC. Steps with continue until the "none" is the top one
