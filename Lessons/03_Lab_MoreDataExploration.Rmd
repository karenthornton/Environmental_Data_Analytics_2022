...
matrix1[4, ] #row
matrix1[ , 3] #column
matrix1[c(12, 14)] #get 12 and 14
matrix1[c(12:14)] #get 12 to 14
matrix1[2:4, 1:3] #
...
> Answer: completes what you want, example the column name
...
```{r}
high <- c(50,58,40,41,44,37,41,51,52,52)
high
low <- c(26,30,24,17,27,22,18,26,26,28)
low
...
```{r}
high_celcius <- function(x){
  ((high-32)*5)/9
}
high_celcius()

low_celcius <- function(x){
  ((low-32)*5)/9
}
low_celcius()
```
...
```{r}
df <-data.frame(high,low,high_celcius(),low_celcius())
colnames(df)<- c("High_F", "Low_F", "High_C", "Low_C")
df
```
