...
#covariance and correlation measure joint variability of two variables (a measure of linear dependence b/t two variables)
#autocorrelation: a measure of dependence b/t two adjacent values of the same variables
#the probability laws that govern stationarity behavior does not change over time
#Mann-Kendall Test: used to detect deterministic trends in series of environmental data, climate data or hydrological data. cannot be applied to seasonal data (null hypothesis is that its stationary)
#spearman correlation coefficient is a statistical measure of the strength of a monotonic relationship, relationship doesn't have to be linear (unlike pearson), if the correlation is close to 0, then there is no trend
#dick-fuller test: the first work on testing for a unit root in time series, null hypothesis is that alpha is equal to 1 (i.e contain a unit root), alternative hypothesis is alpha is less than 1 (i.e. is stationary)
#more general dick-fuller is ADF


#correlation: how are they relate, increase together, decrease together, one increase and one decrease, between -1 and 1
#auto correlation: how series is related to things previously in the same series. y1 and yn-1
#partial auto correlation: relationship when the influence of intermediate values are eliminated
...
#if you want to see the first 10 in the time series
#head(dataset,10)
```
...
#call the package if there are two calls from separate packages.
