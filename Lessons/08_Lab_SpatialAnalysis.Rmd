...
huc8_sf_filtered <- st_read('./Data/Spatial/NCHUC8.shp') %>%
...
ggplot()+
  geom_sf(data = huc8_sf_filtered, aes(fill = SUBBASIN))
#figure out how to get the subsection map thing
...
HUC12_wind <- st_read('https://raw.githubusercontent.com/ENV859/EnviroAtlasData/main/Wind_Energy.csv') %>% 
  colClasses = c("HUC12" = "factor") %>%
...
#not working because something above is wrong
...
huc2_utm <- st_transform()
  
#not going to work because data is from lesson and not in lab
```
...
selected_HUCS <- huc8_sf_utm[triangle_sf,]
...
clipped_HUCs <- huc8_sf_utm %>%
  st_intersection(triangle_sf)
